...
Usage:
  ...
  scripts/scout-vrt.sh test [--filter <string>]... [--stateful | --serverless=<es|oblt|security>]
...
Notes:
  ...
  - 'test' discovers Playwright configs via 'node scripts/scout discover-playwright-configs' and runs each with 'node scripts/scout run-tests'.
    - Passes: '--stateful' (default) or '--serverless=<es|oblt|security>'.
...
run_tests_command() {
  ...
  local -a filters=()
  local server_mode="stateful" # values: stateful | serverless=es|oblt|security
  local grep_tag="@ess"         # values: @ess | @svlSearch | @svlOblt | @svlSecurity
  while [ $# -gt 0 ]; do
    case "$1" in
      --filter=*)
        {
      ...
      --serverless=*)
        ...
        ;;
      -h|--help)
        echo "Usage: scripts/scout-vrt.sh test [--filter <string>]... [--stateful | --serverless=<es|oblt|security>]" >&2
        return 0
        ;;
      *)
  ...
  while IFS= read -r config_path; do
    ...
    echo "[$ran/$total] SCOUT_VISUAL_REGRESSION_ENABLED=true ${run_cmd[*]}"
    if ! SCOUT_VISUAL_REGRESSION_ENABLED=true "${run_cmd[@]}"; then
