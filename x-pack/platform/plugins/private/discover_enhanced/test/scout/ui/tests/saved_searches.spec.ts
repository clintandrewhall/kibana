...
spaceTest.describe(
  ...
  () => {
    ...
    spaceTest(
      ...
      async ({ pageObjects, page, visualRegression }) => {
        await pageObjects.dashboard.goto();
        await visualRegression.capture('go to dashboard');
        ...
        await visualRegression.capture('customize panel');
        await expect(
    ...
    spaceTest(
      ...
      async ({ pageObjects, page, visualRegression }) => {
        ...
        await pageObjects.discover.waitForHistogramRendered();
        await visualRegression.capture('save search');
        ...
        await visualRegression.capture('click new search');
        await assertDataViewIsSelected(page, testData.DATA_VIEW_NAME.ECOMMERCE);
        ...
        await visualRegression.capture('select logstash data view');
        await assertNoFilterAndEmptyQuery(filterFieldAndValue, pageObjects, page);

        // change data view again
        await pageObjects.discover.selectDataView(testData.DATA_VIEW_NAME.ECOMMERCE);
        await visualRegression.capture('select ecommerce data view again');
        await assertNoFilterAndEmptyQuery(filterFieldAndValue, pageObjects, page);

        // create new search again
        await pageObjects.discover.clickNewSearch();
        await visualRegression.capture('click new search again');
        await assertDataViewIsSelected(page, testData.DATA_VIEW_NAME.ECOMMERCE);
      }
    );
  }
