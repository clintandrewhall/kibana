    ...
    return options.map((option) => {
      ...
      return {
        ...
        view: renderOption(option, {
          checked,
          isActive,
          onClick: (e: MouseEvent) => handleOptionClick(value, e),
        }),
      };
    });
  }, [options, selection, renderOption, handleOptionClick]);
  ...
  const handleSelectChange = useCallback(
    (updatedOptions: Array<InternalSelectableOption<T>>) => {
      if (singleSelection) {
        ...
        if (newlyChecked) {
          toggleValue(newlyChecked.value, 'include');
        } else {
          const unchecked = updatedOptions.find(
            (item) => item.checked !== 'on' && prevCheckedByKey.get(item.key) === 'on'
          );
          if (unchecked) {
            toggleValue(unchecked.value, 'include');
          }
        }
      } else {
        ...
        if (changed) {
          toggleValue(changed.value, 'include');
        }
      }
    },
