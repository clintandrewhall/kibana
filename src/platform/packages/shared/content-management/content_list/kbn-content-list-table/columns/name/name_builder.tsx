 ...
 * - `showStarred` defaults to `true` only if `supports.favorites` is `true`.
 *
...
export const buildColumn: ColumnBuilder<SupportedColumnsConfig['name']> = (
  ...
  // Show starred by default only if favorites feature is supported (enabled on provider).
  // Can be explicitly set to true/false to override.
  const showStarred = customConfig.showStarred ?? context.supports?.favorites ?? false;
  ...
  const sortable =
    customConfig.sortable ?? context.sortableFields?.includes('title') ?? true;
