...
describe('ContentListProvider', () => {
  ...
  describe('Feature support flags', () => {
    ...
    it('should set supports.favorites to true when favorites service is provided', async () => {
      ...
      } as unknown as ContentListProviderProps['services']['favorites'];
      ...
      expect(result.current.supports.favorites).toBe(true);
    });

    it('should set supports.favorites to false when favorites service is missing', async () => {
      const { result } = await renderHookWithProvider(() => useContentListConfig(), {
        providerOverrides: {
          services: {},
        },
      ...
      expect(result.current.supports.favorites).toBe(false);
    });

    it('should set supports.favorites to false when favorites is explicitly disabled', async () => {
      const mockFavoritesService = {
        favoritesClient: {
          getFavorites: jest.fn(),
          addFavorite: jest.fn(),
