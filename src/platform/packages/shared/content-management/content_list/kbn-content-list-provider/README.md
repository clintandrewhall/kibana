...
The package exports two providers:

- **`ContentListKibanaProvider`** — For Kibana applications. Adapts Kibana services (tags, favorites, user profiles) to the generic provider interface.
- **`ContentListProvider`** — For non-Kibana contexts like Jest tests, Storybook, or other Elastic applications. Accepts explicit service implementations.

This separation allows the same rendering components to work in any environment by swapping providers.
...
const findItems: FindItemsFn = async ({ searchQuery, filters, sort, page, signal }) => {
  // Route through favorites endpoint when favoritesOnly is enabled.
...
> **Note:** For Jest tests and Storybook, use `ContentListProvider` with mock services instead of `ContentListKibanaProvider`. See [Jest and Storybook](#jest-and-storybook).

> **More examples:** For additional patterns like search boxes, pagination, custom filtering, and more, see [Recipes](./RECIPES.mdx).

## Usage

### Kibana Applications

For typical Kibana usage, wrap your content list components with `ContentListKibanaProvider`:
...
const DashboardListPage = () => {
  ...
  return (
    <ContentListKibanaProvider
      ...
      core={core}
      favorites={favorites}
      // tags, userProfiles, and favorites are automatically enabled
...
**Feature defaults:**
- Service-dependent features (`tags`, `userProfiles`, `favorites`) are automatically enabled when their service is provided.
...
const wrapper = ({ children }) => (
  <ContentListProvider
    ...
    // tags and favorites are automatically enabled since services are provided
  >
    {children}
  </ContentListProvider>
);
...
- `ContentListKibanaProvider` accepts Kibana service dependencies (`savedObjectsTagging`, `core`, `favorites`) and internally wires up the appropriate sub-providers.
- `ContentListProvider` accepts a `services` prop with explicit implementations, enabling the same functionality without Kibana dependencies.
...
dataSource={{
  findItems: async ({ searchQuery, filters, sort, page, signal }) => {
    // Handle favoritesOnly filter via favorites search endpoint.
    if (filters.favoritesOnly) {
      return favoritesClient.search({
        savedObjectType: 'dashboard',
        search: searchQuery,
...
| `filters` | `ActiveFilters` | Active filters including `tags`, `users`, `favoritesOnly`, and custom filters. |
...
|---------|------------------|-------------|
| `tags` | `savedObjectsTagging` | Tag filtering and display. |
| `userProfiles` | `core.userProfile` | User profile filtering. |
| `favorites` | `favorites` service | Favorites functionality. |

```tsx
// Simple: services provided = features enabled
<ContentListKibanaProvider
  savedObjectsTagging={taggingService}    // tags automatically enabled
  core={coreStart}                        // userProfiles automatically enabled
  favorites={favoritesService}            // favorites automatically enabled
  // ...
/>
...
- **[Recipes](./RECIPES.mdx)** — Common patterns and examples for building content list UIs, including basic components, custom fields, sorting, and filtering.
