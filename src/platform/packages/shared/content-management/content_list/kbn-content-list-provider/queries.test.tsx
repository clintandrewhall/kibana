...
describe('useContentListItemsQuery', () => {
  ...
  describe('parseSearchQuery integration', () => {
    it('should use parseSearchQuery to extract tags from query text', async () => {
      const mockItems = createMockItems(10);
      const mockFindItems = jest.fn(createMockFindItems(mockItems));
      const parseSearchQuery = jest.fn((query: string) => ({
        searchQuery: 'clean query',
        tagIds: ['tag-1', 'tag-2'],
      ...
      const { result } = renderHook(
        () =>
          useContentListItemsQuery({
            dataSource,
            entityName: 'test',
            searchQueryText: 'tag:important test',
            filters: {},
            sort: { field: 'title', direction: 'asc' },
            page: { index: 0, size: 10 },
            parseSearchQuery,
      ...
      expect(parseSearchQuery).toHaveBeenCalledWith('tag:important test');
      const callArgs = mockFindItems.mock.calls[0][0] as FindItemsParams;
      expect(callArgs.searchQuery).toBe('clean query');
      expect(callArgs.filters.tags?.include).toEqual(['tag-1', 'tag-2']);
    });

    it('should handle tag exclusion from parseSearchQuery', async () => {
      const mockItems = createMockItems(10);
      const mockFindItems = jest.fn(createMockFindItems(mockItems));
      const parseSearchQuery = jest.fn((query: string) => ({
        searchQuery: 'clean query',
        tagIds: undefined,
        tagIdsToExclude: ['tag-archived'],
      }));
  ...
  describe('query parsing from text', () => {
    it('should extract is:favorite from query text', async () => {
      const mockItems = createMockItems(10);
      const mockFindItems = jest.fn(createMockFindItems(mockItems));
      const dataSource = {
        findItems: mockFindItems,
      ...
      const { result } = renderHook(
        () =>
          useContentListItemsQuery({
            dataSource,
            entityName: 'test',
            searchQueryText: 'is:favorite test query',
            filters: {},
            sort: { field: 'title', direction: 'asc' },
            page: { index: 0, size: 10 },
          }),
      ...
      const callArgs = mockFindItems.mock.calls[0][0] as FindItemsParams;
      expect(callArgs.filters.favoritesOnly).toBe(true);
      // Clean query should not include is:favorite
      expect(callArgs.searchQuery).not.toContain('is:favorite');
    });
