...
export interface FilterDisplayState {
  ...
  /** Whether starred filtering should be shown (based on `supports.favorites`). */
  hasStarred: boolean;
}
...
export const useFilterDisplay = (): FilterDisplayState => {
  ...
  if (typeof filtering === 'object') {
    ...
    hasStarred = supports.favorites && filtering?.favorites !== false;
