 ...
 *
 * Includes known filter types (`tags`, `users`, `favorites`) and extensible custom filters.
 *
 * @example
 * ```json
 * {
 ...
 *   favoritesOnly: true,            // Favorites filter is active
...
export interface ActiveFilters {
   ...
   * This is the search portion without filter syntax (tag:, is:favorite, etc.)
  ...
  users?: string[];
  /** Whether to only include favorites in results */
 ...
 *     favorites: true,                                     // Enable favorites filter
 *     custom: {
 *       status: {
 *         name: 'Status',
 *         options: [
