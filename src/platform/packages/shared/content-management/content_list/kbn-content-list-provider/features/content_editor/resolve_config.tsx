...
export interface ResolveContentEditorDeps {
  ...
  /** Optional content insights client for activity view. */
...
export const resolveContentEditorConfig = (
  ...
  const { core, savedObjectType, entityNamePlural, contentInsightsClient } = deps;
  ...
  return {
    // Use provided onSave or fall back to default.
    onSave: config.onSave ?? createDefaultOnSave(core, savedObjectType),
    // Pass through appendRows as-is (no auto-wiring - requires server-side setup).
    appendRows: config.appendRows,
