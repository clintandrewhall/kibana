...
import { getAllCustomFilterKeys, buildQuerySchema, parseQueryFilters } from '../../utils';

/**
 * Hook to access and control filtering functionality
 *
 * Use this hook when you need to read or update filters (tags, users, favorites, etc.).
 *
...
export const useContentListFilters = () => {
  ...
  // Parse tags, favorites, users, custom filters, and search text from query text.
  const queryFilters: ActiveFilters = useMemo(() => {
    ...
    // Parse `is:favorite`, `createdBy:xxx`, and custom fields from query text.
    const parsed = parseQueryFilters(searchText, querySchema, customFilterKeys, {
      logErrors: true,
    });
    const favoritesOnly = parsed.favoritesOnly || undefined;
    const users = parsed.users;
    const customFilters = parsed.customFilters;
    searchText = parsed.cleanSearchQuery;
  ...
  // Query-derived filters (tags, favorites, search, custom) override state filters
  ...
  const filters: ActiveFilters = useMemo(() => {
    const merged: ActiveFilters = {
      ...
      favoritesOnly: queryFilters.favoritesOnly ?? state.filters.favoritesOnly,
    };
