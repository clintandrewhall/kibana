...
describe('defaultTransform', () => {
  it('should transform basic UserContentCommonSchema to ContentListItem', () => {
    ...
    expect(result.canFavorite).toBeUndefined(); // undefined = use provider default
  });
  ...
  it('should set canFavorite to undefined for non-managed items (opt-out model)', () => {
    const input: UserContentCommonSchema = {
      id: 'test-id-favorite',
      type: 'dashboard',
      attributes: {
        title: 'Favoritable Dashboard',
      },
      updatedAt: '2024-01-15T10:30:00Z',
      references: [],
      managed: false,
    ...
    // undefined means "use provider default" - show favorites if provider supports it
    expect(result.canFavorite).toBeUndefined();
  });

  it('should set canFavorite to false for managed items', () => {
    const input: UserContentCommonSchema = {
      id: 'test-id-managed',
      type: 'dashboard',
      attributes: {
    ...
    // Managed items explicitly cannot be favorited
    expect(result.canFavorite).toBe(false);
  });
