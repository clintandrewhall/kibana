...
/**
 * Demo component for filtering by tags using `useContentListFilters`.
 *
...
const TagControls = () => {
  ...
  const activeIncludes = new Set(filters.tags?.include ?? []);
  ...
  const handleTagClick = (tag: Tag) => {
    ...
    const current = filters.tags?.include ?? [];
    const next = activeIncludes.has(tag.id)
      ? current.filter((id) => id !== tag.id)
      : [...current, tag.id];

    const currentExclude = filters.tags?.exclude ?? [];
    const hasInclude = next.length > 0;
    const hasExclude = currentExclude.length > 0;

    setFilters({
      ...filters,
      tags: hasInclude || hasExclude ? { include: next, exclude: currentExclude } : undefined,
    });
  };
