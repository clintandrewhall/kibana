...
import { ContentListProvider, type FindItemsResult, type FindItemsParams } from '../../../../index';
...
describe('useContentListUserFilter', () => {
  ...
  it('updates selectedUsers when setSelectedUsers is called', () => {
    ...
    act(() => {
      result.current.setSelectedUsers(['user-1', 'user-2']);
    });
  ...
  it('clears users when setSelectedUsers is called with empty array', () => {
    const { result } = renderHook(() => useContentListUserFilter(), {
      wrapper: createWrapper({ withService: true }),
    });

    act(() => {
      result.current.setSelectedUsers(['user-1']);
    });

    expect(result.current.hasActiveFilter).toBe(true);

    act(() => {
      result.current.setSelectedUsers([]);
    });
