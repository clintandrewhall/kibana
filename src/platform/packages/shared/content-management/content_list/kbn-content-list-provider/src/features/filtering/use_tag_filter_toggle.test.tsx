...
describe('useTagFilterToggle', () => {
  ...
  describe('include (withModifierKey: false)', () => {
    it('adds a tag to include filters', () => {
      ...
      expect(result.current.filters.filters.tags).toEqual({
        include: ['tag-1'],
        exclude: [],
      });
    });
    ...
    it('removes a tag from include on second call (toggle off)', () => {
      ...
      expect(result.current.filters.filters.tags).toBeUndefined();
    });
    ...
    it('removes the tag from exclude when adding to include', () => {
      ...
      expect(result.current.filters.filters.tags?.exclude).toContain('tag-1');
      ...
      expect(result.current.filters.filters.tags?.include).toContain('tag-1');
      expect(result.current.filters.filters.tags?.exclude).not.toContain('tag-1');
    });
    ...
    it('accumulates multiple include tags', () => {
      ...
      expect(result.current.filters.filters.tags?.include).toEqual(['tag-1', 'tag-2']);
    });
  });
  ...
  describe('exclude (withModifierKey: true)', () => {
    ...
    it('removes the tag from include when adding to exclude', () => {
      ...
      expect(result.current.filters.filters.tags?.include).not.toContain('tag-1');
      expect(result.current.filters.filters.tags?.exclude).toContain('tag-1');
    });
  });
  ...
  describe('normalization', () => {
    it('returns `undefined` tags when both include and exclude become empty', () => {
      ...
      expect(result.current.filters.filters.tags).toBeDefined();
